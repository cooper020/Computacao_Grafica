...
Point produtoVetorial(Point a, Point b) {
    Point res = {
            a.y * b.z - a.z * b.y,
            a.z * b.x - a.x * b.z,
            a.x * b.y - a.y * b.x
...
void plane(float length, int divisions, string file) {
    ...
    for (int i = 0; i < divisions; i++) {
        for (int j = 0; j < divisions; j++) {
            ...
            normal = produtoVetorial(point2, point3);
            normalize(&normal);
            point1.nx = normal.x; point1.ny = normal.y; point1.nz = normal.z;
            point2.nx = normal.x; point2.ny = normal.y; point2.nz = normal.z;
            point3.nx = normal.x; point3.ny = normal.y; point3.nz = normal.z;

            normal = produtoVetorial(point4, point3);
            normalize(&normal);
            point4.nx = normal.x; point4.ny = normal.y; point4.nz = normal.z;
            point2.nx = normal.x; point2.ny = normal.y; point2.nz = normal.z;
            point3.nx = normal.x; point3.ny = normal.y; point3.nz = normal.z;
...
void cone(float radius, float height, int slices, int stacks, string file) {
    ...
    for (int i = 0; i < stacks; i++) {
        for (int j = 0; j < slices; j++) {
            ...
            float nx = stackRadius * sinf(alpha);
            float ny = sinf(atan(radius / height));
            float nz = stackRadius * cosf(alpha);

            point1 = {stackRadius * sin(alpha), i * (height / stacks), stackRadius * cos(alpha), nx, ny, nz, (float)j / slices, (float)i / stacks};
            point2 = {stackRadius2 * sin(alpha), (i + 1) * (height / stacks), stackRadius2 * cos(alpha), nx, ny, nz, (float)j / slices, (float)(i + 1) / stacks};
            point3 = {stackRadius2 * sin(alpha2), (i + 1) * (height / stacks), stackRadius2 * cos(alpha2), nx, ny, nz, (float)(j + 1) / slices, (float)(i + 1) / stacks};
            point4 = {stackRadius * sin(alpha2), i * (height / stacks), stackRadius * cos(alpha2), nx, ny, nz, (float)(j + 1) / slices, (float)i / stacks};
...
void torus(float innerRadius, float outerRadius, int slices, int stacks, string file) {
    ...
    for (int i = 0; i < slices; i++) {
        ...
        for (int j = 0; j < stacks; j++) {
            ...
            point1 = {(outerRadius + innerRadius * cos(beta1)) * cos(alpha1), innerRadius * sin(beta1), (outerRadius + innerRadius * cos(beta1)) * sin(alpha1), -cos(beta1) * cos(alpha1), -sin(beta1), -cos(beta1) * sin(alpha1), (float)i / slices, (float)j / stacks};
            point2 = {(outerRadius + innerRadius * cos(beta2)) * cos(alpha1), innerRadius * sin(beta2), (outerRadius + innerRadius * cos(beta2)) * sin(alpha1), -cos(beta2) * cos(alpha1), -sin(beta2), -cos(beta2) * sin(alpha1), (float)i / slices, (float)(j + 1) / stacks};
            point3 = {(outerRadius + innerRadius * cos(beta2)) * cos(alpha2), innerRadius * sin(beta2), (outerRadius + innerRadius * cos(beta2)) * sin(alpha2), -cos(beta2) * cos(alpha2), -sin(beta2), -cos(beta2) * sin(alpha2), (float)(i + 1) / slices, (float)(j + 1) / stacks};
            point4 = {(outerRadius + innerRadius * cos(beta1)) * cos(alpha2), innerRadius * sin(beta1), (outerRadius + innerRadius * cos(beta1)) * sin(alpha2), -cos(beta1) * cos(alpha2), -sin(beta1), -cos(beta1) * sin(alpha2), (float)(i + 1) / slices, (float)j / stacks};
